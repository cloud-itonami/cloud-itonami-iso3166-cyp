  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Lefkosia Fabrication Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Lefkosia Fabrication Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest self-cleaning-unsupported-is-held-and-unoverridable
  (testing "declared :exclusion-ground? true and :invokes-self-cleaning? true, but the cumulative Article 57(6) evidence is incomplete -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:self-cleaning-unsupported} (-> (store/ledger db) last :basis)))
...
(deftest tax-record-unverified-is-held-and-unoverridable
  (testing "unverified Tax Department/Tax Portal record when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:tax-record-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
