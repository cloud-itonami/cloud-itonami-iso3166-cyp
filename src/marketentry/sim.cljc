...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (CYP, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up self-cleaning-unsupported) ==")
    ...
    (println "== filing/submit eng-4 (self-cleaning-unsupported -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up tax-record-unverified) ==")
    ...
    (println "== filing/submit eng-5 (tax-record-unverified -> HARD hold) ==")
